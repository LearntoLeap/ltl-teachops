...
API có tải ảnh/tài liệu (tối đa 15MB) và video minh chứng (tối đa 100MB) và có luồng thông báo thời gian thực (SSE).
Nginx mặc định sẽ chặn/đệm hai thứ này. Vào **Website** → site vừa tạo →
**Conf** (hoặc **Config File**), thêm vào trong khối `server { ... }`:

```nginx
# Tài liệu tối đa 15MB, video tối đa 100MB — nới giới hạn mặc định 1MB của Nginx
...
| Tải ảnh/video báo lỗi 413 | Thiếu `client_max_body_size 110m;` trong cấu hình Nginx (Bước 4) |
