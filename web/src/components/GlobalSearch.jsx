...
function Row({ icon, title, sub, onPick }) {
  return (
    <button type="button" onClick={onPick}
      ...
      <span className="text-[15px] w-6 text-center shrink-0">{icon}</span>
    ...
    <div className="py-1">
      {groups.map((g) => (
        <div key={g.key} className="px-2 pb-1">
          <div className="px-2 pt-2 pb-1 text-[10.5px] font-bold uppercase tracking-[.1em] text-ink-muted">
            {g.label}
          </div>
          {g.rows.map((r, i) => (
    ...
    <div ref={boxRef} className="relative w-[300px] xl:w-[360px]">
      <div className="flex items-center gap-2 rounded-full bg-white border border-line px-3.5 py-[7px]
        ...
        <input
          ...
          className="flex-1 min-w-0 bg-transparent outline-none text-[13.5px] placeholder:text-ink-muted/70"
        />
      ...
      {open && (
        <div className="absolute top-[calc(100%+6px)] inset-x-0 z-50 card shadow-card-lg max-h-[420px] overflow-y-auto">
          {data && q.trim().length >= 2 ? (
            ...
            <div className="py-2">
              <div className="px-4 pt-1.5 pb-1 text-[10.5px] font-bold uppercase tracking-[.1em] text-ink-muted">
                {recent.length ? 'Tìm gần đây' : 'Gợi ý'}
              </div>
              {(recent.length ? recent : ['Tên trường', 'Tên giáo viên', 'Lớp 5A', 'Giáo án khối 6']).map((s) => (
                <button key={s} type="button"
                  onClick={() => { onChange(s); }}
                  className="w-full text-left px-4 py-1.5 text-[13px] text-ink-soft hover:bg-brand-50/60">
                  {recent.length ? '🕘 ' : '💡 '}{s}
                </button>
              ))}
            </div>
