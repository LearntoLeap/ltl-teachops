  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="flex items-center gap-2 card px-4 py-3 sticky top-2 z-10">
        ...
        {q && (
          <button className="text-ink-muted text-[15px] px-1" onClick={() => onChange('')} aria-label="Xoá">✕</button>
        )}
      </div>
      ...
      <div className="card mt-3 overflow-hidden">
        {data && q.trim().length >= 2 ? (
          ...
          <div className="py-2">
            <div className="px-4 pt-2 pb-1 text-[10.5px] font-bold uppercase tracking-[.1em] text-ink-muted">
            ...
            {(recent.length ? recent : ['Tên trường', 'Tên giáo viên', 'Lớp 5A', 'Giáo án khối 6']).map((s) => (
              <button key={s} type="button" onClick={() => onChange(s)}
                className="w-full text-left px-4 py-2 text-[14px] text-ink-soft hover:bg-brand-50/60">
                {recent.length ? '🕘 ' : '💡 '}{s}
              </button>
            ))}
          </div>
