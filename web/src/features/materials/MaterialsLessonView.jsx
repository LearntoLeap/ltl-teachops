                    ...
                    <div className="grid gap-1.5">
                      {l.items.map((m) => (
                        <div key={m.id} className="flex items-center gap-2 rounded-xl border border-line px-2.5 py-1.5 hover:border-brand-300">
                          <span className="text-lg shrink-0">{m.type_icon || '📄'}</span>
                          <button type="button" onClick={() => onPreview(m)}
                            className="min-w-0 flex-1 text-left truncate text-sm font-medium text-ink hover:text-brand-800 hover:underline">
                            {m.title}
                          </button>
                          {m.type_name && (
                            <span className="hidden sm:inline text-xs text-ink-muted shrink-0">{m.type_name}</span>
                          )}
                          ...
                          <span className="shrink-0 flex items-center">
                            ...
                            <Link to={`/hoc-lieu/${m.id}`} title="Mở chi tiết"
                              className="h-10 w-10 sm:h-8 sm:w-8 rounded-lg text-sm text-ink-muted hover:bg-brand-50 hover:text-brand-800 grid place-items-center">›</Link>
                          </span>
                        </div>
                      ))}
                    </div>
